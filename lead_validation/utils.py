...
def validate_and_store_lead_data(lead, save_to_db=True):
    ...
    # Get the lead ID if it exists (for revalidation)
    lead_id = getattr(lead, 'id', None)
    print(f"Current lead ID: {lead_id}")
    ...
    # STEP 2: Always use AI validation
    try:
        logger.info("STEP 2: Running AI validation")
        print("\n*** CALLING AI VALIDATOR ***\n")
        ...
        quality_score = 100 - ai_risk_score
        print(f"AI risk score: {ai_risk_score}, converted quality score: {quality_score}")
